...
class SchemaProcessor(BaseProcessor):
    ...
    def reorder_dataframe(self, df: Any, status_print_fn: Any = print) -> Any:
        """
        Reorder DataFrame columns according to the schema sequence.
        
        Args:
            df: Pandas DataFrame to reorder.
            status_print_fn: Function for status messages.
        ...
        ordered_col_names = [col for col in ordered_columns.keys() if col in df.columns]

        # Append any columns in DataFrame but not in schema sequence
        extra_cols = [col for col in df.columns if col not in ordered_col_names]
        final_col_order = ordered_col_names + extra_cols

        if extra_cols:
            status_print_fn(f"Warning: {len(extra_cols)} column(s) not in column_sequence: {extra_cols}")

        # Reorder DataFrame
        df_reordered = df[final_col_order]
        status_print_fn(f"Columns reordered: {len(ordered_col_names)} columns in schema order")
        status_print_fn(f"Total columns: {len(df_reordered.columns)}")
