...
# Default error codes mapping for validation rule types.
# Can be overridden via schema_data['validation_error_codes'].
# Based on error_handling/config/error_codes.json
DEFAULT_VALIDATION_ERROR_CODES = {
    'required': 'V5-I-V-0505',
    'allow_null': 'V5-I-V-0505',
    'pattern': 'V5-I-V-0501',
    'min_length': 'V5-I-V-0501',
    'max_length': 'V5-I-V-0502',
    'min_value': 'V5-I-V-0501',
    'max_value': 'V5-I-V-0501',
    'format': 'V5-I-V-0504',
    'allowed_values': 'V5-I-V-0503',
    'schema_reference_check': 'V5-I-V-0506',
...
ERROR_CODES = DEFAULT_VALIDATION_ERROR_CODES
        ...
        for validation in validation_rules:
            ...
            if rule_type == 'pattern':
                ...
                if mask.any():
                    ...
                    code = ERROR_CODES.get('pattern', 'V5-I-V-0501')
...
def apply_validation(df: pd.DataFrame, columns_schema: dict, schema_data: dict,
    ...
    # C1: Read validation error codes from schema_data if available (SSOT).
    # Falls back to DEFAULT_VALIDATION_ERROR_CODES so existing behaviour is preserved.
    error_codes = schema_data.get('validation_error_codes', DEFAULT_VALIDATION_ERROR_CODES)
    ...
    for column_name, column_def in columns_schema.items():
        ...
        if not allow_null and mask_null.any():
            code = error_codes.get('allow_null', 'V5-I-V-0505')
        ...
        for validation in validation_rules:
            ...
            if rule_type == 'min_length' or ('min_length' in validation and rule_type is None):
                ...
                if allow_null:
                    mask &= df_validated[column_name].notna()
                if mask.any():
                    code = error_codes.get('min_length', 'V5-I-V-0501')
            ...
            if rule_type == 'max_length' or ('max_length' in validation and rule_type is None):
                ...
                if allow_null:
                    mask &= df_validated[column_name].notna()
                if mask.any():
                    code = error_codes.get('max_length', 'V5-I-V-0502')
            ...
            if rule_type == 'min_value' or ('min_value' in validation and rule_type is None):
                ...
                if allow_null:
                    mask &= df_validated[column_name].notna()
                if mask.any():
                    code = error_codes.get('min_value', 'V5-I-V-0501')
            ...
            if (rule_type == 'format' or ('format' in validation and rule_type is None)) and validation.get('format') == 'YYYY-MM-DD':
                ...
                if allow_null:
                    mask &= df_validated[column_name].notna()
                if mask.any():
                    code = error_codes.get('format', 'V5-I-V-0504')
            ...
            if rule_type == 'allowed_values' or ('allowed_values' in validation and rule_type is None):
                ...
                if allow_null:
                    mask &= df_validated[column_name].notna()
                if mask.any():
                    code = error_codes.get('allowed_values', 'V5-I-V-0503')
            ...
            if rule_type == 'group_consistency':
                ...
                if group_cols and target_col and target_col in df_validated.columns:
                    ...
                    if len(valid_group_cols) == len(group_cols):
                        ...
                        if mask.any():
                            code = error_codes.get('group_consistency', 'V5-I-V-0501')
            ...
            if rule_type == 'schema_reference_check':
                ...
                if schema_ref:
                    ...
                    if mask is not None and mask.any():
                        code = error_codes.get('schema_reference_check', 'V5-I-V-0506')
                ...
                if schema_ref:
                    ...
                    if allowed_codes:
                        ...
                        if allow_null:
                            mask &= df_validated[column_name].notna()
                        if mask.any():
                            code = error_codes.get('starts_with_schema_reference', 'V5-I-V-0501')
            ...
            if rule_type == 'derived_pattern':
                ...
                if mask.any():
                    code = error_codes.get('derived_pattern', 'V5-I-V-0501')
