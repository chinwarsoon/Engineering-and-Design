...
| Aspect | Details |
| :----- | :------ |
| **Inputs** | Engineering documents (PDF, DOCX, XLSX, DWG, DGN); project asset datadrop (Excel, 7 sheets, 7,681 items); JSON schema definitions (22 files in `eks/config/schemas/`) |
| **Outputs** | DuckDB document registry (`documents` + `document_elements` tables, auto-generated DDL); 22 schema/config JSON files; parsed document metadata; health scores; structural element detection; manual review queue; project setup validation |
