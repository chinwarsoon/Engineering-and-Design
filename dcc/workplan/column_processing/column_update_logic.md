...
| 20 | `All_Submission_Sessions` | **P3** | Calculated | `Submission_Session`, `Document_ID` | **aggregate/concatenate_unique**: Group by `Document_ID`, join unique sessions with `"&&"` | Calculation FIRST, null handling LAST if needed |
| 21 | `All_Submission_Dates` | **P3** | Calculated | `Submission_Date`, `Document_ID` | **aggregate/concatenate_dates**: Group by `Document_ID`, coerce to datetime, sort chronologically, format `YYYY-MM-DD`, join with `", "` | Calculation FIRST, null handling LAST if needed |
| 22 | `All_Submission_Session_Revisions` | **P3** | Calculated | `Submission_Session_Revision`, `Document_ID` | **aggregate/concatenate_unique**: Group by `Document_ID`, join unique revisions with `", "` | Calculation FIRST, null handling LAST if needed |
...
| 33 | `All_Approval_Code` | **P3** | Calculated | `Approval_Code`, `Document_ID`, `Submission_Date` | **aggregate/concatenate_unique**: Group by `Document_ID`, sort by `Submission_Date`, join unique approval codes with `", "` | Calculation FIRST, null handling LAST if needed |
