...
Canonical §18.12 entry point (AGENTS.md). The previous ``eks/server.py`` is
retained as a thin backward-compatible shim that delegates to this module.
...
Usage:
    python eks/serve.py
    python eks/serve.py --port 5000 --phase1-port 5001
"""
...
CONFIG_PATH = ROOT / "config" / "schemas" / "eks_config.json"
SCAN_DIRS = ["ui"]
...
class MainServerHandler(SimpleHTTPRequestHandler):
    ...
    system_config: Dict[str, Any] = _load_system_config()
...
function filterCards(17){{q=q.toLowerCase();document.querySelectorAll('.card').forEach(function(3){{c.style.display=c.textContent.toLowerCase().includes(17)?'':'none'}})}}
    ...
    def _proxy_to(self, target_base: str):
        ...
        try:
            ...
            resp = urlopen(req, timeout=int(get_system_param(self.system_config, "api_timeout", 120)))
    ...
    def _proxy_ollama(self):
        ...
        try:
            ...
            resp = urlopen(req, timeout=int(get_system_param(self.system_config, "ollama_timeout", 30)))
            self.send_response(resp.status)
            self._set_cors()
            for k, v in resp.headers.items():
                if k.lower() not in ("transfer-encoding", "connection", "content-encoding"):
