...
class FillDetector(BaseDetector):
    ...
    # Error codes
    ERROR_JUMP_LIMIT_A = "F4-C-F-0401-A"
    ERROR_JUMP_LIMIT_B = "F4-C-F-0401-B"
    ERROR_BOUNDARY_CROSS_A = "F4-C-F-0402-A"
    ERROR_BOUNDARY_CROSS_B = "F4-C-F-0402-B"
    ERROR_FILL_INFERRED_A = "F4-C-F-0403-A"
    ERROR_FILL_INFERRED_B = "F4-C-F-0403-B"
    ERROR_FILL_INFERRED_C = "F4-C-F-0403-C"
    ...
    def _check_forward_fill_record(self, record: Dict) -> None:
        ...
        if row_jump > self.jump_limit:
            ...
            self.detect_error(
                error_code=self.ERROR_JUMP_LIMIT_A,
                message=self._format_message(self.ERROR_JUMP_LIMIT_A),
                row=to_row_idx,
                column=column,
                fail_fast=False,
                additional_context={
        ...
        if record.get("session_boundary_crossed", False):
            # Phase D: Enhanced error context
            self.detect_error(
                error_code=self.ERROR_BOUNDARY_CROSS_A,
                message=self._format_message(self.ERROR_BOUNDARY_CROSS_A),
                row=to_row_idx,
                column=column,
                fail_fast=False,
                additional_context={
    ...
    def _check_multi_level_record(self, record: Dict) -> None:
        ...
        if record.get("all_levels_failed", False):
            ...
            self.detect_error(
                error_code=self.ERROR_FILL_INFERRED_A,
                message=self._format_message(self.ERROR_FILL_INFERRED_A),
                row=to_row_idx,
                column=column,
                fail_fast=False,
                additional_context={
                ...
                if pd.notna(current_value) and current_value != '':
                    if prev_value is not None and current_value == prev_value:
                        ...
                        if row_jump > self.jump_limit:
                            ...
                            self.detect_error(
                                error_code=self.ERROR_JUMP_LIMIT_B,
                                message=self._format_message(self.ERROR_JUMP_LIMIT_B),
                                row=idx,
                                column=col,
                                fail_fast=False,
                                additional_context={
            ...
            for idx in df.index:
                ...
                if pd.notna(current_value) and current_value != '':
                    if (prev_value is not None and 
                        ...
                        self.detect_error(
                            error_code=self.ERROR_BOUNDARY_CROSS_B,
                            message=self._format_message(self.ERROR_BOUNDARY_CROSS_B),
                            row=idx,
                            column=col,
                            fail_fast=False,
                            additional_context={
        ...
        for col, source_cols in inferred_columns.items():
            ...
            for idx in df.index:
                ...
                if pd.notna(value) and value != '':
                    ...
                    if sources_missing:
                        self.detect_error(
                            error_code=self.ERROR_FILL_INFERRED_B,
                            message=self._format_message(self.ERROR_FILL_INFERRED_B),
                            row=idx,
                            column=col,
                            fail_fast=False,
                            additional_context={
    ...
    def _check_default_value_record(self, record: Dict) -> None:
        ...
        if default_applied:
            ...
            self.detect_error(
                error_code=self.ERROR_FILL_INFERRED_C,
                message=self._format_message(self.ERROR_FILL_INFERRED_C),
                row=to_row_idx,
                column=column,
                fail_fast=False,
                additional_context={
        ...
        for column, stats in column_fill_stats.items():
            ...
            if fill_percentage > self.max_fill_percentage:
                ...
                self.detect_error(
                    ...
                    message=self._format_message(self.ERROR_EXCESSIVE_NULLS, column=column, fill_percentage=fill_percentage),
                    row=to_row_idx,
                    column=column,
                    fail_fast=False,
                    additional_context={
    ...
    def _detect_invalid_grouping(self, record: Dict) -> None:
        ...
        if group_by and len(group_by) == 0:
            ...
            self.detect_error(
                ...
                message=self._format_message(self.ERROR_INVALID_GROUPING, column=column),
                row=to_row_idx,
                column=column,
                fail_fast=False,
                additional_context={
