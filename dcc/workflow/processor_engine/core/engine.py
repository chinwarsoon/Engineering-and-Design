...
from core_engine.logging import DEBUG_LEVEL, log_context
...
from utility_engine.console import debug_print, status_print
...
class CalculationEngine(BaseProcessor):
    ...
    def __init__(
        ...
        if business_detector is None:
            ...
            business_detector = BusinessDetector(
                enable_fail_fast=fail_fast, logger=self.structured_logger
            )
        ...
        if error_reporter is None:
            ...
            output_dir = getattr(
                self.context.paths, "csv_output_path", Path("output")
            ).parent
        ...
        if column_sequence:
            ...
            self.columns = {
                name: defn
                for name, defn in raw_columns.items()
    ...
    def process_data(self, df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        with log_context("processor", "process_data"):
            ...
            total_rows = len(df)
            status_print(
                f"🚀 Starting processing of {total_rows:,} rows...", min_level=1
            )

            # Use phased processing (P1 → P2 → P2.5 → P3)
            df = self.apply_phased_processing(
                df, heartbeat=heartbeat, total_rows=total_rows
            )

            # Phase 3: Emit final heartbeat summary
            final_payload = heartbeat.final_summary(
                total_rows, status_print_fn=status_print
            )
            self.context.telemetry.heartbeat_logs.append(final_payload.to_dict())

            debug_print(
                f"Finished phased processing, now has {len(df.columns)} columns"
            )
    ...
    def apply_phased_processing(
        ...
        with log_context("processor", "apply_phased_processing"):
            ...
            parameters = (
                self.context.blueprint.validation_rules
                or self.schema_data.get("parameters", {})
            )
            df_processed = initialize_missing_columns(
                df_processed, self.columns, parameters
            )
            ...
            phase_order = self.schema_data.get(
                "processing_phase_order", ["P1", "P2", "P2.5", "P3"]
            )

            # Configuration map for phased processing steps
            phase_config = {
                "P1": {
                    "method": self._apply_phase_null_handling,
                    "enum": ProcessingPhase.P1,
                    "desc": "Meta Data",
                },
                "P2": {
                    "method": self._apply_phase_transactional,
                    "enum": ProcessingPhase.P2,
                    "desc": "Transactional",
                },
                "P2.5": {
                    "method": self._apply_phase_calculated,
                    "enum": ProcessingPhase.P2_5,
                    "desc": "Anomaly",
                },
                "P3": {
                    "method": self._apply_phase_calculated,
                    "enum": ProcessingPhase.P3,
                    "desc": "Calculated",
                },
            }
            ...
            for phase_id in phase_order:
                ...
                debug_print(f"[PHASED] Getting columns for phase {phase_id}")
                phase_cols = self.context.blueprint.get_columns_by_phase(phase_id)
                debug_print(
                    f"[PHASED] Got {len(phase_cols)} columns for phase {phase_id}"
                )
                ...
                if phase_id == "P2.5":
                    ...
                    detection_context["fill_history"] = getattr(
                        self, "fill_history", []
                    )

                # Run business detection for this phase
                phase_results = self.business_detector.detect(
                    df_processed, context=detection_context, phases=[config["enum"]]
                )
            ...
            self._print_processing_step(
                "Phase 4", "Validation", "Applying all schema validation rules"
            )
            from ..calculations.validation import apply_validation

            schema_data_full = self.schema_data
            df_processed = apply_validation(
                df_processed, self.columns, schema_data_full
            )

            # Phase 4: Row Validation - Cross-field business logic
            self._print_processing_step(
                "Phase 4", "RowValidation", "Running row-level cross-field checks"
            )
            ...
            validation_errors_col = next(
                (
                    c
                    for c in p3_cols
                    if self.columns.get(c, {}).get("calculation", {}).get("type")
                    == "error_tracking"
                ),
                "Validation_Errors",
            )
            self._print_processing_step(
                "Phase 4", "Aggregation", f"Populating {validation_errors_col} column"
            )
            df_processed[validation_errors_col] = (
                self.error_aggregator.format_validation_errors_column(len(df_processed))
            )

            # Phase 5: Metrics (Layer 5) - Populate Data_Health_Score column (Step 48)
            health_score_col = next(
                (
                    c
                    for c in p3_cols
                    if self.columns.get(c, {}).get("column_type") == "score_column"
                ),
                "Data_Health_Score",
            )
            self._print_processing_step(
                "Phase 5", "Metrics", f"Calculating {health_score_col}"
            )
            from reporting_engine.core.report_health import calculate_row_health_series

            df_processed[health_score_col] = calculate_row_health_series(
                df_processed, self.error_aggregator
            )
    ...
    def _apply_phase_null_handling(
        ...
        debug_print(
            f"[NULL-HANDLER] Starting with {len(column_names)} columns, df shape: {df.shape}"
        )
        debug_print(
            f"[NULL-HANDLER] Columns to process: {column_names[:5]}{'...' if len(column_names) > 5 else ''}"
        )

        df_result = df.copy()
        debug_print(f"[NULL-HANDLER] DataFrame copied successfully")

        for i, column_name in enumerate(column_names):
            debug_print(
                f"[NULL-HANDLER] Processing column {i + 1}/{len(column_names)}: {column_name}"
            )

            if column_name not in df_result.columns:
                debug_print(
                    f"[NULL-HANDLER] Column '{column_name}' not in DataFrame, skipping"
                )
                continue

            if column_name not in self.columns:
                debug_print(
                    f"[NULL-HANDLER] Column '{column_name}' not in schema, skipping"
                )
                continue
            ...
            debug_print(f"[NULL-HANDLER] Strategy for {column_name}: {strategy}")

            if strategy and strategy != "leave_null":
                handler = get_null_handler(strategy)
                if handler:
                    debug_print(
                        f"[NULL-HANDLER] Calling handler for {column_name} with strategy {strategy}"
                    )
                    df_result = handler(self, df_result, column_name, null_handling)
                    debug_print(f"[NULL-HANDLER] Handler completed for {column_name}")
                else:
                    debug_print(
                        f"[NULL-HANDLER] No handler found for strategy {strategy}"
                    )
            else:
                debug_print(
                    f"[NULL-HANDLER] Skipping {column_name} - no strategy or leave_null"
                )

        debug_print(
            f"[NULL-HANDLER] Finished null handling phase, result shape: {df_result.shape}"
        )
        return df_result

    def _apply_phase_transactional(
        self, df: pd.DataFrame, column_names: List[str]
    ) -> pd.DataFrame:
        """
        ...
        for column_name in column_names:
            ...
            if strategy and strategy != "leave_null":
                ...
                if handler:
                    self._print_processing_step(
                        "P2-ForwardFill", column_name, f"Applying {strategy}"
                    )
                    df_result = handler(self, df_result, column_name, null_handling)

        return df_result

    def _apply_phase_calculated(
        self, df: pd.DataFrame, column_names: List[str]
    ) -> pd.DataFrame:
        """
        ...
        for column_name in column_names:
            ...
            if strategy and strategy.null_handling_timing in (
                NullHandlingTiming.BUILT_IN,
                NullHandlingTiming.SKIP,
            ):
                self._print_processing_step(
                    "Strategy-Null",
                    column_name,
            ...
            if (
                strategy_name
                and strategy_name != "leave_null"
                and df_result[column_name].isna().any()
            ):
                ...
                if handler:
                    self._print_processing_step(
                        "Last-Defense",
                        column_name,
                        f"Filling remaining nulls with {strategy_name}",
                    )
