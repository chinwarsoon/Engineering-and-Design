...
| `version` | `1.2.0` | Current version of the ontology configuration. |
...
| `CONNECTS_TO` | — | ✅ | ✅ | Connectivity between functional objects. |
...
| `CONTROLS` | — | ❌ | ❌ | Control signal relationship. |
| `FEEDS_FROM` | — | ❌ | ❌ | Material or energy feed relationship. |
...
5.  **`tag_type_aliases`** (Array, Optional): List of alternative codes (e.g., `AT_PMP` for `AT_EQPMP`).
6.  **`fragments`** (Array, Optional): List of asset schema fragments required by instances of this class.
